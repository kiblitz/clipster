...
impl VimState {
    ...
    fn _set_mode(&mut self, mode: VimMode) {
        self.mode = mode;
    }

    fn _push_buffer(&mut self, c: char) {
        self.buffer.push(c);
        // check buffer match
        // if buffer match, clear buffer
    }
}
...
impl<'a> App<'a> {
    ...
    pub fn on_key(&mut self, c: char, _with_ctrl: bool) {
        match c {
            'q' => {
